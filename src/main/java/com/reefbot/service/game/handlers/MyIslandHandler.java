...
public class MyIslandHandler implements GameHandler {
    ...
    public static BotResponse buildScreen(Player player, Island island) {
        ...
        // Resources
        sb.append("\n📦 Ресурсы\n")
          .append("🐟 Рыба: ").append(island.getFish())
          .append("  🐚 Ракушки: ").append(island.getShells())
          .append("  🪸 Коралл: ").append(island.getCoral()).append("\n")
          .append("🪵 Дерево: ").append(island.getWood())
          .append("  🪨 Камень: ").append(island.getStone()).append("\n")
          .append("Хранилище: ").append(totalResources(island)).append("/").append(island.getStorageCapacity());
            ...
            if (zone.getMinDevPoints() > 0 && !zone.isUnlocked(dp)) {
                if (firstLocked) {
                    ...
                    locked.append(zone.getDisplayName())
                          .append(" — 🔒 ").append(zone.getMinDevPoints())
                          .append(" ОР (сейчас: ").append(dp).append(")\n")
                          .append("   ").append(zoneTeaser(zone));
                    firstLocked = false;
                } else {
                    locked.append("\n").append(zone.getDisplayName())
                          .append(" — 🔒 ").append(zone.getMinDevPoints()).append(" ОР");
                }
            }
        }
        if (locked.length() > 0) {
